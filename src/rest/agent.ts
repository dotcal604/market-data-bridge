...
const actions: Record<string, ActionHandler> = {
  ...
  // ── Holly Analytics Dashboard (pre-computed from Python pipeline) ──
  holly_analytics_dashboard: async () => {
    const __dirname = dirname(fileURLToPath(import.meta.url));
    const jsonPath = resolve(__dirname, "../../analytics/output/holly_dashboard.json");
    if (!existsSync(jsonPath)) {
      throw new Error("holly_dashboard.json not found. Run: python analytics/output/generate_dashboard_json.py");
    }
...
export const actionsMeta: Record<string, ActionMeta> = {
  ...
  holly_analytics_dashboard: {
    description: "Pre-computed analytics dashboard: filter impact, equity curves, TOD/regime/strategy performance, YoY trends. Generated by Python pipeline from 28K+ enriched trades.",
  },
