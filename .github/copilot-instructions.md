...
## AI Collaboration Channel (Session Log)

The collab channel is a persistent decision/context log used by Claude Code and ChatGPT. Copilot agents cannot reach it (runs in GitHub sandbox, no localhost access).

- **Read messages:** `GET /api/collab/messages` — query params: `limit`, `author`, `type` (info/decision)
- **Post message:** `POST /api/collab/message` — body: `{ author, content, type?, tags?, metadata? }`

**Message types:** `info` (status update), `decision` (architectural or implementation choice).
